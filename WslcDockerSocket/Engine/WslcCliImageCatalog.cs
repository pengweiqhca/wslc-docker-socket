...
internal sealed class WslcCliImageCatalog(IWslcCommandRunner runner)
{
    ...
    private static bool TryParseTimestamp(string? text, out long unixSeconds)
    {
        ...
        // WSLC renders Go's "2006-01-02 15:04:05 -0700 MST"; the trailing zone name is not parsable by .NET.
    ...
    private static long ReadSizeBytes(JsonElement image)
    {
        ...
        return TryParseHumanizedSize(GetOptionalString(image, "Size"), out var bytes) ? bytes : 0;
    }

    private static bool TryParseHumanizedSize(string? text, out long bytes)
    {
        bytes = 0;
        if (string.IsNullOrWhiteSpace(text)) return false;
        ...
        // Go humanizes image sizes with decimal units; binary suffixes such as MiB step by 1024 instead.
